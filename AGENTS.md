...
这是 `awesome-harness-engineering`（`ahe`）个人工作台，用于维护可复用的 harness 资产、skills、模板、规则草案和研究文档。工作流类 skill 以 **AHE workflow skills** 形式放在扁平目录 `ahe-coding-skills/ahe-*` 下；其他自有 skill 仍按需放在 `ahe-coding-skills/<skill-name>/`。
...
3. **从局部入口继续深入**：查看 skill 时从 `ahe-coding-skills/<skill-name>/SKILL.md` 开始（workflow 族为 `ahe-coding-skills/ahe-*/SKILL.md`）；查看 workflow 共享文档时从 `ahe-coding-skills/docs/` 开始；查看 workflow 相关模板时从 `ahe-coding-skills/templates/` 开始；查看其他通用模板时从 `templates/` 开始；查看其他长文资料时从 `docs/` 开始。
