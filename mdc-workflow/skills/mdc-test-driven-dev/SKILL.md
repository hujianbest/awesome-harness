...
description: 作为 MDC 系列唯一的实现入口使用。适用于任务计划已批准后的实现阶段，或 `mdc-hotfix` 进入受控修复时；本 skill 负责单任务推进、测试设计确认、TDD 执行以及把结果交给后续 review/gate。当前默认覆盖 C++/GoogleTest 场景；若项目不是 C++，需显式说明当前未覆盖，而不是假定存在其他实现。
---
...
- 在 full / standard profile 中，`mdc-tasks-review` 通过后进入本 skill
- 在 lightweight profile 中，主链实现也直接进入本 skill
- `mdc-hotfix` 进入复现与最小修复时，也统一进入本 skill
- 本 skill 完成后，默认把结果交给 `mdc-bug-patterns`、`mdc-test-review`、`mdc-code-review`、`mdc-traceability-review`、`mdc-regression-gate`、`mdc-completion-gate`
- 对 lightweight profile，可按 workflow 约定跳过中间 review 层，直接进入 `mdc-regression-gate`
...
- 本 skill 内部再执行测试设计确认、TDD、实现、状态更新和后续交接
...
若当前 profile 是 lightweight，则按 starter 已确定的裁剪链路执行，通常直接进入：

1. `mdc-regression-gate`
2. `mdc-completion-gate`

## workflow 默认顺序

```text
实现入口 -> 测试用例设计 -> 真人确认测试设计 -> Red-Green-Refactor -> 缺陷模式排查 -> 测试评审 -> 代码评审 -> 可追溯性评审 -> 回归门禁 -> 完成门禁
```
