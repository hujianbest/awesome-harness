...
- **INVEST Validation**: 检查每个任务是否满足 Independent/Negotiable/Valuable/Estimable/Small/Testable，防止不可执行任务流入实现。
- **Dependency Graph Validation**: 校验任务间依赖关系的正确性和无环性，确认关键路径合理。
- **Traceability Matrix**: 检查任务是否忠实覆盖规格/设计的每一项关键决策，防止覆盖缺口。
- **Structured Walkthrough**: 多维度评分量化判断，任一关键维度低于阈值不得通过。
