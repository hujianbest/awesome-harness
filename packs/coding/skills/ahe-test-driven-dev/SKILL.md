...
- **TDD (Test-Driven Development, Beck/Kent Beck)**: 严格遵循 Red → Green → Refactor 循环。先写失败测试定义期望行为，再写最小实现使测试通过，最后重构。
- **Walking Skeleton (Cockburn)**: 优先建立最薄端到端可运行路径，确保架构验证在全面实现之前完成。
- **Test Design Before Implementation**: 在 Red-Green-Refactor 前完成测试设计 approval step，确保测试策略经过审视而非即兴产生。
- **Fresh Evidence Principle**: 所有验证证据必须在当前会话内产生，不接受历史证据或口头确认替代实际运行结果。
