...
- **WBS (Work Breakdown Structure)**: 自顶向下将设计拆解为可管理的任务层级，每个任务有明确范围、不重叠、可分配。
- **INVEST Criteria**: 任务粒度检查遵循 Independent/Negotiable/Valuable/Estimable/Small/Testable 六维度，防止过大或模糊任务。
- **Dependency Graph + Critical Path**: 显式建模任务间依赖关系，识别关键路径，确保执行顺序可验证。
- **Definition of Done (Scrum)**: 每个任务具备可判断的完成条件，从模糊"实现某模块"收敛到可观察、可验证的行为目标。
