...
适用：regression gate 之后需判断当前任务完成后的走向；确认任务可宣告完成；准备更新 task-progress/RELEASE_NOTES；用户要求"能不能算完成"。
...
记录保存到 `docs/verification/completion-<task>.md`。结构包含：结论、已消费上游结论、上游证据矩阵、完成宣告范围、剩余任务判断、已验证结论、证据、覆盖边界、明确不在范围内的项、下一步。
