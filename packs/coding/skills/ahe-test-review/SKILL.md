...
- **Fail-First Validation (TDD Quality Gate)**: 验证测试确实先失败再通过，防止"天生绿色"的无效测试冒充可信验证。
- **Coverage Categories (Crispin/Gregory)**: 从行为覆盖、风险覆盖、边界覆盖等多维度评估测试质量，而非仅看行覆盖率数字。
- **Bug-Pattern-Driven Testing**: 测试覆盖必须回应 bug-patterns 识别的风险，确保高危路径有针对性保护。
- **Structured Walkthrough**: 多维度评分量化判断，防止印象式评审。
