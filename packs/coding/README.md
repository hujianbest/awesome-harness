...
`packs/coding/` 是 Garage 自带的 **HarnessFlow（HF）工程工作流 family pack**，把 product discovery → hypothesis experiment → spec → design → tasks → TDD 实现 → 多维 review → gate → finalize 的完整工程链路打包成可分发的 23 个 skill + 15 个 family-level 共享资产。
...
| `version` | `0.2.0` |
| `schema_version` | `1` |
| `skills` | 23（22 hf-* + 1 using-hf-workflow） |
| `agents` | 0 |
| family-level 资产 | 15（4 docs + 6 templates + 5 principles） |
...
### `packs/coding/principles/` — 5 个 family-level principles
...
23 个 skill 内任意 `skills/docs/<file>` / `templates/<file>` / `references/<file>` 形式相对引用，在 `packs/coding/` 内必须 resolve 到磁盘存在的真实文件。
...
本仓库自身贡献者首次 clone 后，需要先跑一次 dogfood 才能在 IDE 内加载这 23 个 skill：
...
本 pack 内容 reverse-sync 自 [`hujianbest/harness-flow`](https://github.com/hujianbest/harness-flow) upstream。Phase 0 同步时（version 0.2.0）：
