...
在完整 AHE workflow 中，它通常用于正式评审链中的追溯性判断。
...
- 当前任务是否需要回到实现修订，还是可以进入 `ahe-regression-gate`
- 当前阻塞原因、断链点或待补同步项（如有）
...
`进入 ahe-regression-gate` | `回到 ahe-test-driven-dev` | `补齐阻塞条件后重试 ahe-traceability-review`
