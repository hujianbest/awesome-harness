...
description: 对当前改动执行回归门禁。既可作为完整 MDC workflow 中的回归质量能力，也可独立用于确认相关行为、更大范围测试、构建或检查未被破坏。
---
...
在完整 MDC workflow 中，它通常用于正式完成判断之前；如果当前工作没有走整条 workflow，它也可以独立使用。
...
如果是独立使用，无法提供完整任务计划时，至少应提供当前改动范围、需要保持不变的行为，以及可执行的回归验证命令。
...
如果是独立使用，允许把结论直接输出在当前对话、评审评论或临时验证记录中，但仍必须基于最新证据给出明确门禁结论、回归风险和唯一下一步。
