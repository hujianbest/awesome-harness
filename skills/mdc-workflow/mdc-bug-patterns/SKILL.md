...
description: 基于团队历史错误案例和常见缺陷模式，对当前改动做专项排查。既可作为完整 MDC workflow 中的质量能力，也可独立用于高风险改动、重复缺陷类型、复杂状态处理或热修复后的定向排查。
---
...
在完整 MDC workflow 中，它通常作为实现后的首个质量能力；如果当前工作并未使用整条 workflow，它也可以单独使用。
...
- 需要独立对当前改动做一轮高风险专项排查，不要求先走完整 MDC workflow
...
如果是独立使用，无法提供完整规格或任务文档时，至少应提供当前改动范围、目标行为和本次排查关注点。
...
如果是独立使用，允许把结论直接输出在当前对话、评审评论或临时评审记录中，但仍必须给出明确结论、命中模式、缺失防护和唯一下一步。
