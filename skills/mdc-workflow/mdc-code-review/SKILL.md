...
description: 对已实现任务的代码进行评审。既可作为完整 MDC workflow 中的实现质量能力，也可独立用于检查正确性、可读性、错误处理、设计一致性和局部代码质量。
---
...
在完整 MDC workflow 中，它通常用于正式评审链中的实现质量判断；如果当前工作没有走整条 workflow，它也可以独立使用。
...
如果是独立使用，无法提供完整规格、设计或测试评审结论时，至少应提供当前改动范围、目标行为和可参考的测试或验证证据。
...
如果是独立使用，允许把结论直接输出在当前对话、评审评论或临时评审记录中，但仍必须给出明确结论、发现项、代码风险和唯一下一步。
