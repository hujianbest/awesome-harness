...
- 能力型 skill：负责完成某一类具体检查或评审，可被 workflow 调度，也可在上下文充分时单独调用
...
但如果用户已经明确要求某个能力型 skill，且当前上下文足以支撑该能力直接执行，也可以直接调用对应 skill，而不必强制先走入口路由。
...
| `mdc-implement` | 路由型 | 按唯一活跃任务执行实现，并把实现结果交给后续质量能力 | 任务计划已批准，且仍有未完成任务 |
...
| `mdc-increment` | 路由型 | 处理需求追加、范围调整与变更影响同步 | 用户提出新增、删改需求，或现有工件表明范围变化 |
| `mdc-hotfix` | 路由型 | 处理紧急缺陷修复，并要求先复现再修 | 用户提出紧急缺陷修复，或现有证据表明处于热修复场景 |
| `mdc-finalize` | 路由型 | 收尾当前工作周期，更新状态、发布说明和证据索引 | 完成门禁通过后，准备结束当前周期时 |

## 各 Skill 详细说明

这一节补充每个 `mdc-*` skill 的目标、典型输入输出与关键约束。README 不重复 `SKILL.md` 的全部细节，但会说明每个 skill 在 workflow 中的职责、可否独立调用以及与其它 skill 的边界。
...
- 可由 workflow 调用，也可在高风险改动、热修复或复发缺陷场景中独立调用
...
- 可由 workflow 调用，也可在需要单独评估测试资产质量时直接调用
...
- 可由 workflow 调用，也可在需要独立判断实现质量时直接调用
...
- 可由 workflow 调用，也可在需要独立核对工件链路和验证链路时直接调用
...
- 可由 workflow 调用，也可在需要独立确认回归面是否健康时直接调用
...
- 可由 workflow 调用，也可在需要独立判断“是否真的可以宣告完成”时直接调用
...
如果用户已经明确要求某个能力型 skill，且当前输入足以支撑它直接完成工作，可以直接调用对应能力型 skill，不必为了形式先回到入口路由。
...
### 3. review-only 请求一定要先走 `mdc-workflow-starter` 吗？

不一定。

如果用户已经明确要求某一种能力型 review，且上下文足以支撑该 review 直接执行，可以直接进入对应 skill。

但如果以下任一问题仍不清楚，仍应先走 `mdc-workflow-starter`：

- 当前到底该做哪一种 review
- 当前是否真的处在合适的 review 时机
- 当前 review 结论会如何影响主链或支线推进
