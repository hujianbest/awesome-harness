...
description: 检查需求、设计、任务、实现、测试与验证证据之间是否仍然一致。既可作为完整 MDC workflow 中的追溯性质量能力，也可独立用于确认当前改动没有无记录偏离、设计漂移或验证断链。
---
...
在完整 MDC workflow 中，它通常用于正式评审链中的追溯性判断；如果当前工作没有走整条 workflow，它也可以独立使用。
...
- 需要独立确认当前改动的规格、设计、任务、实现和验证是否还能互相回指
...
如果是独立使用，无法提供完整已批准工件时，至少应提供当前改动范围、目标行为和可核对的一组需求 / 设计 / 任务 / 验证依据。
...
如果是独立使用，允许把结论直接输出在当前对话、评审评论或临时评审记录中，但仍必须给出明确结论、追溯缺口、漂移风险和唯一下一步。
