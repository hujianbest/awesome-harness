...
只要当前请求属于软件交付流程中的任一场景，且你还需要判断当前阶段、核对工件状态或决定下一步，就应先经过这里：

- 开始一个新需求、功能或项目
- 用户说“继续”“推进”“开始做”“先处理这个”
- 用户要求看代码、澄清需求、做设计、拆任务或直接实现
- 用户只要求做某种 review，但尚不清楚该进入哪个评审能力或是否处在合适时机
- 用户提出需求变更
- 用户提出紧急缺陷修复

如果当前阶段尚未完成这里的路由判断，就不要先进入下游路由型 skill，也不要先开始推进主链或支线工作。

如果用户已经明确要求某个能力型 skill，且当前上下文已经足够让它独立完成工作，则可以直接调用对应能力型 skill，不必强制回到入口。

## 目的

你的职责不是直接实现，而是判断当前所处阶段、识别 workflow 中推荐的下一步 skill，并阻止乱序推进。
...
对 workflow 场景而言，澄清、探索、读代码、review、设计、任务拆解、实现都属于推进动作，推进之前必须先完成路由。
...
| “用户只是让我做 review，不算进入流程” | 如果 review 类型、时机或上下文不清楚，仍要先做路由判断。 |
...
6. 若用户明确要求“只做测试评审 / 代码评审 / 追溯性评审”，且该能力所需上下文已足够，进入对应 review skill
...
后续若进入能力型 skill，应由该 skill 直接完成本类工作；只有当阶段判断再次变得不清楚，或需要恢复主链 / 支线推进时，才回到本入口重新路由。
