...
description: 评审 MDC 任务计划。既可作为完整 MDC workflow 中的任务质量能力，也可独立用于检查粒度、顺序、依赖正确性、可验证性以及是否足以指导实现。
---
...
在完整 MDC workflow 中，它通常用于实现前的任务质量判断；如果当前工作没有走整条 workflow，它也可以独立使用。
...
如果是独立使用，允许把结论直接输出在当前对话、评审评论或临时评审记录中，但仍必须给出明确结论、计划薄弱点和唯一下一步。
