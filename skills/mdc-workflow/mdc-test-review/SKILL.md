...
description: 对已实现任务的测试进行评审。既可作为完整 MDC workflow 中的测试质量能力，也可独立用于检查新增或变更测试是否体现 fail-first、覆盖关键行为并真正有用。
---
...
在完整 MDC workflow 中，它通常用于正式评审链中的测试质量判断；如果当前工作没有走整条 workflow，它也可以独立使用。
...
如果是独立使用，无法提供完整规格或设计时，至少应提供测试目标、当前实现范围和可用的失败证据或替代说明。
...
如果是独立使用，允许把结论直接输出在当前对话、评审评论或临时评审记录中，但仍必须给出明确结论、测试质量缺口和唯一下一步。
