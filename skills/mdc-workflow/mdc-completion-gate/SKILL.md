...
description: 使用最新验证证据对当前任务执行最终完成门禁。既可作为完整 MDC workflow 中的末端质量能力，也可独立用于判断当前任务是否已经具备宣告完成、更新状态或切换任务的条件。
---
...
在完整 MDC workflow 中，它通常作为末端完成判断；如果当前工作没有走整条 workflow，它也可以独立使用。
...
如果是独立使用，无法提供完整评审链记录时，至少应提供当前任务的完成声明、支持该声明的最新验证命令，以及可用的结果证据。
...
如果是独立使用，允许把结论直接输出在当前对话、评审评论或临时验证记录中，但仍必须用最新证据批准或拒绝完成结论，并给出唯一下一步。
