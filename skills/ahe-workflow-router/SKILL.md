...
4. 若 `reroute_via_starter=true` 或 handoff 明确要求重新编排，直接按 router authority 重判
...
当前仓库中的 runtime 参考资料仍暂时存放在：

- `skills/ahe-workflow-starter/references/`

因此当前阶段遵守：

- write-time canonical: `ahe-workflow-router`
- read-time alias accepted: `ahe-workflow-starter`
- 旧文档、旧 handoff、旧 reviewer return 中出现 starter 时，先按 compatibility alias 理解
