...
- `ahe-workflow-starter` 仍作为 compatibility alias 被接受

新的 runtime 语义应优先以 `ahe-workflow-router` 为 canonical 名称；旧写法 `ahe-workflow-starter` 在迁移期仍可读。
...
4. 若 `reroute_via_router=true`（历史字段 `reroute_via_starter=true` 与之等价）或 handoff 明确要求重新编排，直接按 router authority 重判
...
- write-time canonical: `ahe-workflow-router`
- read-time alias accepted: `ahe-workflow-starter`（skill 名与 handoff 文本）
- 旧文档、旧 handoff、旧 reviewer return 中出现 `ahe-workflow-starter` 时，先按 compatibility alias 理解
- 旧 reviewer 摘要若仍写 `reroute_via_starter`，与 `reroute_via_router` 等价读取
...
- [ ] 新的 runtime canonical 语义优先写 `ahe-workflow-router`，同时兼容读取 `ahe-workflow-starter`
