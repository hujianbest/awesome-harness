...
description: 作为 AHE 系列唯一的实现入口使用。适用于任务计划已批准后的实现阶段，或受控修复已进入实现阶段时；本 skill 负责单任务推进、测试设计确认、TDD 执行，以及把 fresh evidence、风险和推荐下一步写回工件。当前默认覆盖 C++/GoogleTest 场景；若项目不是 C++，需显式说明当前未覆盖，而不是假定存在其他实现。
---
...
- 若项目不是 C++，明确说明当前 `ahe-test-driven-dev` 仅已覆盖 C++，需要后续补对应语言实现
