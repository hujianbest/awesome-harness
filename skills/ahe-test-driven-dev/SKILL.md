...
description: 作为 AHE 系列唯一的实现入口使用。适用于任务计划已批准后的实现阶段，或受控修复已进入实现阶段时；本 skill 负责单任务推进、测试设计确认、TDD 执行，以及把 fresh evidence、风险和推荐下一步写回工件。通用实现规则适用于所有技术栈，若仓库内提供了 C++/GoogleTest 具体规范，则按附录细化执行。
---
...
- 本 skill 完成后，不在本节点内部决定下游质量节点；由 `ahe-workflow-starter` 按当前 profile 恢复后续编排
...
- 若项目是 C++ / GoogleTest / CMake 测试栈，按本文通用规则 + 后续 C++ 附录执行
- 若项目不是 C++，仍按本文通用实现规则执行，并优先使用 `AGENTS.md` 中声明的测试 / 构建 / 验证入口
- 若仓库既不是 C++，`AGENTS.md` 也没有提供必要的实现与验证入口，则应明确报告当前缺少执行所需约定，而不是假定不存在实现路径
...
这里引用的验证结果必须是当前轮最新执行得到的 fresh evidence；不要把旧的绿测结果包装成这次任务的完成证据。
...
然后由 `ahe-workflow-starter` 按当前 profile 恢复后续编排：

- full / standard 通常先判断是否进入 `ahe-bug-patterns`
- lightweight 通常先判断是否进入 `ahe-regression-gate`
...
| “现在已经差不多了，可以先说完成” | 整个 workflow 完成要等评审、回归、完成门禁和收尾都走完；本节点只在实现完成并写回 fresh evidence 后才算节点完成。 |
