...
不适用：
- 写/修代码 → `ahe-test-driven-dev`
- 需求变更/范围调整，或现有工件只能证明“想新增能力” → `ahe-increment`
- 阶段不清、route / stage / worktree 证据冲突，或既有契约仍不清楚 → `ahe-workflow-router`
- 当前已完成 hotfix 分析，只剩实现 → `ahe-test-driven-dev`
...
- 更像 increment：`ahe-increment`
- route / stage / worktree / 既有契约仍冲突：`ahe-workflow-router`
