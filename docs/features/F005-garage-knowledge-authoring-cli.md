...
| ID 生成 | 未传 `--id` 时由 CLI 生成稳定可读 ID：`<type-prefix>-<YYYYMMDD>-<6位 short hash of topic+content>`；传了就用传入值 |
...
- **验收标准**:
  ...
  - Given 用户**未**传 `--id`，Then CLI 生成 `<type-prefix>-<YYYYMMDD>-<6 hex chars>` 形式 ID，6-hex 来自 `sha256(topic + content)[:6]`，保证可复算
...
### FR-507 `experience show` / `delete` 必须支撑最小 CRUD 闭环

- **优先级**: Should
- **来源**: § 4.1（CRUD 对称性）；§ 2.2 SC-2
- **需求陈述**:
  - `garage experience show --id <I>` shall 读取 `.garage/experience/records/<I>.json`，stdout pretty-print JSON，退出码 0；不存在则退 1 + stderr `Experience record '<I>' not found`。
  - `garage experience delete --id <I>` shall 删除文件 + 从 `.garage/experience/records/.metadata/index.json`（若存在）摘除该条；不存在则退 1。
- **验收标准**:
  ...
  - Given `exp-1.json` 存在，When `delete`，Then 文件被删，`show` 紧跟着退 1
  - Given `experience` 索引存在且引用 `exp-1`，When `delete`，Then 索引中不再引用 `exp-1`

### FR-508 ID 生成必须可复算且不冲突

- **优先级**: Must
- **来源**: § 4.1 ID 生成；§ 2.2 SC-1
- **需求陈述**: 当用户未传 `--id` 时，the system shall 用 `<type-prefix>-<YYYYMMDD>-<6 hex chars>` 模式生成 ID，6 hex 来自 `sha256((topic + "\n" + content + "\n" + iso8601 second-precision now).encode("utf-8")).hexdigest()[:6]`；时间戳精度到秒，使得**同一秒**内同一 (topic, content) 重复 add 仍出现冲突 → CLI 退 1 + stderr `Entry with id '<id>' already exists; pass --id to override or change inputs`，不覆盖现有文件。
- **验收标准**:
  - Given 同一 topic + content 在不同秒被 `add`，Then 两次得到不同 ID（时间戳不同）
  - Given 同一 topic + content + 同一秒（极端竞态测试可 monkeypatch `datetime.now`）连续 `add`，Then 第 2 次退 1 + stderr 含 `already exists`，磁盘第一条 entry 不被覆盖
  - Given 用户传 `--id custom`，Then ID 直接使用 `custom`，不走 hash 路径
  - Given 用户传 `--id custom` 且 `(type, custom)` 已存在，Then `add` 退 1 + stderr 含 `already exists`；`edit` 路径不受影响
...
- **验收标准**: `pyproject.toml` 在本 cycle 不新增 dependency；`grep -r "import " src/garage_os/cli.py` 在本 cycle 后仍只引用 stdlib + `garage_os.*`。
...
- **ASM-504**：`ExperienceIndex` 已有 `delete()` 或与之等价的对外删除 API；若没有，`hf-design` 阶段需要消化"在 CLI 层走 `FileStorage.delete()` + 手动 reindex" 的 fallback 决策（属设计层而非规格层，不阻塞规格通过）。
...
| OQ-502 | `experience` 删除是否会破坏 `RecommendationService` 索引一致性？ | 阻塞→已答 | 不阻塞。`ExperienceIndex` 的 delete 路径在 F003 已支持冷重建（NFR-303）；CLI 删除时若内存索引已构建，应触发 invalidate 或走 store-level delete API；`hf-design` 阶段定具体路径。 |
