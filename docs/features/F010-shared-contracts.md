...
因此，`F010` 在当前主线里只继续拥有其中一部分：

- contract 家族本身的稳定 capability cut
- 六类 contract 的最小职责、边界与组合关系
- 它们如何为 pack registration、pack binding 和 reference pack calibration 提供共同语言

它不重新定义：

- pack platform 的内部架构
- `Pack Registry` 的 runtime 子系统分工
- `Pack Runtime Binding` 如何在运行时激活具体 pack
...
- `A160`：拥有 pack platform、本地 pack 绑定与 reference pack calibration 的架构边界。
- `F010`：拥有 shared contracts 家族本身的 feature-level 语义。
- `F110`：拥有 reference packs 的共同形状与平台映射关系。
- `D110 / D120`：拥有各自 pack 如何具体占据这些 contracts 的详细设计。
...
- 冻结 shared contracts 家族本身的稳定 capability cut
- 解释 6 类 contract 的职责、边界与组合关系
- 说明这些 contracts 如何为 pack registration、artifact routing、evidence 与 host adaptation 提供共同语言

后续由不同文档继续展开：

- `A160`：定义 pack platform 与 runtime binding 的架构边界
- `F020`：定义 shared contract schemas 的字段形状
- `F110`：定义 reference packs 如何沿着这些 contracts 映射到平台
- `A170 / F120`：定义 bridge seam 如何在既有 contract 组合下成立
- `D110 / D120`：定义具体 packs 如何占据这些 contract 面

如果后续文档让 `F010` 开始拥有 pack platform 的内部架构，或把 bridge 写成新的特权 contract，应以 `A160 / A170` 为准回头修正。
