...
| 功能 | 描述 |
|------|------|
| `garage recommend <query>` | 顶级新子命令；query 拆 tokens → 构造 query-shaped context → 调 `RecommendationService.recommend()` → 打印 top-N |
...
### FR-601 `garage recommend <query>` 必须基于 query 构造 context 并调用现有 RecommendationService

- **优先级**: Must
- **来源**: § 1 摩擦 1（被动 push → 主动 pull）；§ 2.1 核心目标；§ 3.2 场景 1
- **需求陈述**: When 用户调用 `garage recommend <query>`（query 为非空字符串，可选 `--tag <t>` 可重复、`--domain <d>` 单值、`--top <N>` 整数默认 10），the system shall：
  ...
  3. 实例化 `RecommendationService(KnowledgeStore, ExperienceIndex)`
  4. 调 `service.recommend(context)`，按 score 降序取 top-N
  5. stdout 打印每条 `[TYPE] title` / `ID: ...` / `Score: X.X` / `Match: <逗号分隔 match_reasons>` 块，块间空行
  6. 退出码 0
- **验收标准**:
  ...
  - Given 0 条 entry，When `garage recommend "x"`，Then exit 0 + stdout `RECOMMEND_NO_RESULTS_FMT.format(query="x")`，无 entry 块
  - Given 调用方未 init `.garage/`，Then exit 1 + stderr `ERR_NO_GARAGE`（与 F005 一致）

### FR-602 `garage recommend` 必须在零结果时给出明确兜底文案

- **优先级**: Must
- **来源**: § 3.2 场景 3
- **需求陈述**: When `RecommendationService.recommend(context)` 返回空 list 或全部 score=0，the system shall stdout 输出 `RECOMMEND_NO_RESULTS_FMT.format(query=<原始 query>)`（"No matching knowledge or experience for query: '<query>'"），退出码 0。
- **验收标准**:
  - Given 0 条 entry，When `garage recommend "x"`，Then stdout == `No matching knowledge or experience for query: 'x'\n`（仅一行 + 换行）
  - Given 5 条 entry 但 query 完全不命中，Then 同上行为
  - **不**抛 SystemExit 非零、**不**写 stderr

### FR-603 `garage knowledge link` 必须把 to 追加到 from 的 related 边并走 update() 递增 version
...
- **需求陈述**: When 用户调用 `garage knowledge link --from <from-id> --to <to-id>`（可选 `--kind related-decision|related-task` 默认 `related-decision`），the system shall：
  1. 在 3 个 `KnowledgeType` 目录下查找 `(_, from-id)` 唯一匹配（type 不需要用户传，CLI 自动嗅探；多匹配则报错——见 FR-604）
...
### FR-604 `garage knowledge link --from` 必须在多 type 匹配时显式报错
...
- **验收标准**:
  ...
  - Given 仅 `(decision, X)` 存在，则正常按 FR-603 happy path 执行

### FR-605 `garage knowledge graph --id` 必须打印节点 + 出边 + 入边
...
- **验收标准**:
  ...
  - Given `--id` 在多 type 命中（`(decision, X)` + `(pattern, X)`），Then exit 1 + stderr 含 `ERR_LINK_FROM_AMBIGUOUS_FMT`（同 FR-604 共用常量）

### FR-606 link 路径必须延伸 F005 cli: 命名空间约定
...
### FR-607 CLI help 必须可冷读发现新增子命令

- **优先级**: Should
- **来源**: § 2.2 SC-6；`design-principles.md` 原则 5 "约定可发现"
- **需求陈述**:
  - `garage --help` stdout 含 `recommend`
  - `garage knowledge --help` stdout 含 `link` 与 `graph`
...
- **验收标准**: 上述 5 条断言每条对应 1 个 test func（cross-cutting test class）
...
- **验收标准**: `pyproject.toml` 在本 cycle 不新增 runtime dependency；F005 cycle 末态 `git diff main..HEAD -- pyproject.toml` 为空。
...
- **ASM-602**：用户使用 `garage knowledge add` / candidate publisher 写入的 ID 在 3 个 `KnowledgeType` 目录中**全局唯一**（即同名 ID 不会跨 type 同时存在）。F005 ID 生成器 `<type-prefix>-<YYYYMMDD>-<6 hex>` 天然带 type 前缀避免碰撞，但用户可显式 `--id custom` 跨 type 重名。**失效**：FR-604 通过 `ERR_LINK_FROM_AMBIGUOUS_FMT` 显式拒绝；用户必须重命名其中一个。
- **ASM-603**：`KnowledgeEntry.related_decisions` / `related_tasks` 在所有现有 entry 上都是 `List[str]`（默认空 list；F001 dataclass 默认 `field(default_factory=list)`）。**失效**：若历史磁盘 entry 这两字段为 `None`（理论上 `_front_matter_to_entry` 已用 `fm.get(...)` 默认 `[]` 兜底，见 `knowledge_store.py:475-476`），CLI 仍能安全 append；本 cycle 不专门测试这条边界但相信兜底。
...
| OQ-603 | `link --from` 在多 type 命中时报错而非自动选 type 优先级，会不会让用户困惑？ | 非阻塞 | 优先显式拒绝（FR-604）。理由：自动选优先级是隐式行为，违反原则 5 "约定可发现"；显式报错让用户立刻知道问题。如果未来出现高频投诉，可加 `--type` 参数让用户消歧。 |
...
| OQ-606 | `--to` 是否要校验"形如 ID 的格式"？ | 非阻塞 | 否。`--to` 接受任意字符串（FR-603 验收）。理由：`related_tasks` 经常引用 Garage 之外的 task ID（T005 这类），强校验会反过来阻塞使用。 |
