# F110: Garage Reference Packs

- Feature ID: `F110`
- 状态: 草稿
- 日期: 2026-04-11
- 定位: 定义 `Garage` 当前用于校准 `A160` pack platform 的两个 `reference packs`，说明它们与 `Pack Platform` 的映射关系、职责边界、共同形状与后续扩展路径。
- 当前阶段: 完整架构主线，先以当前 reference slice 验证 pack platform
- 关联文档:
  - `docs/GARAGE.md`
...
**在 `A160` 已冻结 pack platform 之后，为什么当前先以 `Coding Pack` 与 `Product Insights Pack` 作为两个 `reference packs`，以及它们应如何校准平台中立的 pack platform 与 shared contracts。**

本文覆盖：

- 两个 `reference packs` 的选择理由
- 它们与 `Pack Platform` / `Shared Contracts` 的映射关系
- 各自拥有的职责与不应泄漏到 core 的领域语义
- 所有未来 pack 应复用的共同形状
- `Writing Pack` 与 `Video Pack` 的扩展路径
- 当前 reference slice 的非目标与主要风险

本文不覆盖：

- pack platform 本身的架构拆解
- pack 内部完整角色清单
- pack 内部完整节点图
- 具体 prompt、模板、目录和文件实现
- host 入口细节或服务化方案

## 2. 什么是当前主线的 `reference pack`

`reference pack` 不是写死在平台里的特权能力，也不是未来唯一合法的能力形态。

它的本质是：

- 当前用于验证 `A160` 中 `Reference Pack Calibration` 是否成立的第一批标准样板
...
当前主线先选择这两个 pack，不是因为它们“最重要”，而是因为它们能形成一条最小但异质的创作闭环。
...
当前先只选这两个 pack，是为了优先验证“少而异质”的架构正确性，而不是追求能力数量。
...
- 它们的作用是校准 `A160` 定义的 pack platform，而不是拥有平台特权

## 5. 两个 reference packs 如何映射到 pack platform

| `Pack Platform` / contract | `Product Insights Pack` 的映射 | `Coding Pack` 的映射 |
| --- | --- | --- |
| `Session / Pack Runtime Binding` | 承接上游问题形成与洞察推进，支持 bridge 前的阶段切换与恢复 | 承接下游构建推进与收尾，支持实现中的 review / verification / resume |
| `Pack Registry` | 注册洞察类 roles、nodes 和 artifact roles | 注册构建类 roles、nodes 和 artifact roles |
...
只要一个新 pack 能自然落进这套共同形状，它就应该主要通过新增 pack 文档和 contract 声明接入 `A160` 定义的 pack platform，而不是修改 core。

当前主线中，`Bridge seam` 不是独立的第七个共享 contract，而是通过：
...
当前主线不要求立刻把 `Writing Pack` 和 `Video Pack` 做出来，但它们的扩展路径应该已经清楚：
...
## 10. 当前 reference slice 不要求的内容

为了保证架构收敛，当前 reference slice 明确不要求：
...
### 11.5 富媒体 pack 过早冲击当前 reference slice 边界

控制点：

- 当前 reference slice 只验证扩展 seam 和引用位，不提前做完整媒体系统
...
- `Markdown-first` / `file-backed`：当前 reference slice 先保证可读、可落盘、可追溯。
- 当前主线克制：先用两个 `reference packs` 证明架构成立，再扩大能力面。

## 14. 这篇文档与其他文档的关系

这篇文档负责：

- 解释为什么当前先由 `Coding Pack` 与 `Product Insights Pack` 充当 `reference packs`
- 冻结 reference packs 的共同形状、平台映射关系与校准价值
- 说明未来新 pack 应沿着什么共同模式接入平台

后续由不同文档继续展开：

- `A160`：定义 pack platform、本地 pack 绑定与 reference pack calibration 的架构边界
- `F010`：定义 shared contracts 的稳定 capability cut
- `A170` / `F120`：定义跨 pack bridge 的架构与 feature 语义
- `D110 / D120`：定义两个 reference packs 的详细设计

如果后续文档让 `F110` 开始拥有 pack platform 的内部架构，或赋予 reference packs 平台特权，应以 `A160` 为准回头修正。
