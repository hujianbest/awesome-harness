...
如果把 `Garage` 的项目定义继续压缩，它其实建立在 5 条更底层的判断上：

- 团队先于工具：用户面对的是一支 AI 团队，而不是一堆模型和工具开关。
- 人定方向，AI 在治理中放大：AI 可以主动执行和主动成长，但不能绕开人的判断与治理边界。
- 扩展与成长并列：系统既要持续接入新能力，也要让团队因为做过的事情而变强。
- 长期连续性先于单轮聪明：`memory / session / skill / evidence` 必须分层，不能混成一个历史桶。
- 成长必须 `evidence-first`、`workspace-first`、`governance-bounded`：任何长期更新都应先有证据、先有 proposal，再进入治理与长期资产面。
