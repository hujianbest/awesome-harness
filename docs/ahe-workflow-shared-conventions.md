...
- 若项目必须使用别名，应在 `AGENTS.md` 中声明一对一映射，并保证 router（及读入时的 legacy `ahe-workflow-starter` 别名）可唯一归一化
...
- 写时必须使用 canonical 值；若读取到旧工件中的旧 starter 写法、坏值或自由文本，运行时容错规则以 `skills/ahe-workflow-router/references/execution-semantics.md` 为准（历史工件若仍引用 `ahe-workflow-starter/.../execution-semantics.md`，按兼容读法处理）
...
- 历史摘要或旧 skill 若仍返回 `reroute_via_starter`，读时应视为与 `reroute_via_router` 同义；新产出应优先写 `reroute_via_router`
...
1. 若 `reroute_via_router=true`（或 legacy `reroute_via_starter=true`），先回到 `ahe-workflow-router`
