...
- branch and orchestration: `ahe-hotfix` / `ahe-increment` / `ahe-workflow-starter`
...
- 写时必须使用 canonical 值；若读取到旧工件中的坏值或自由文本，运行时容错规则以 `skills/ahe-workflow-starter/references/execution-semantics.md` 为准
...
1. 若 `reroute_via_starter=true`，先回到 `ahe-workflow-starter`
...
- route / stage / profile / 上游证据冲突：回到 `ahe-workflow-starter`
...
- [ ] route / stage / profile / 上游证据冲突会回到 `ahe-workflow-starter`
