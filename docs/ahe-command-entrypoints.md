...
2. **Starter remains the routing kernel**
一旦出现 route / stage / profile 不清、工件冲突、批准状态不明，命令必须回到 `ahe-workflow-starter`。

3. **Direct invoke is optional, not mandatory**
命令可以优先尝试 direct invoke 某个具体 skill，但只有在前置条件满足时才允许这样做。

4. **No duplicate machine contract**
命令文档不重新定义 verdict、handoff、progress schema、review return contract；这些统一引用共享文档。

5. **One command, one dominant user intent**
命令应优先服务一个高频意图，而不是变成“万能别名”。

6. **Leaf skill gates still apply**
...
1. 识别命令的主意图
2. 检查最小前置条件是否满足
3. 若当前节点已经足够明确，可 direct invoke 对应 skill
4. 若阶段不清、证据冲突、profile 不稳，回到 `ahe-workflow-starter`
5. 后续编排、暂停点、review dispatch 仍按 live workflow contract 执行

## Command Matrix

| 命令 | 主意图 | 偏向 direct invoke 的节点 | 遇到不确定时回哪里 |
|---|---|---|---|
| `/ahe-spec` | 规格澄清 / 规格修订 / 规格入口 | `ahe-specify` | `ahe-workflow-starter` |
| `/ahe-build` | 当前活跃任务实现 / 受控修订实现 | `ahe-test-driven-dev` | `ahe-workflow-starter` |
| `/ahe-review` | review / gate 请求分发 | 具体 review / gate 节点 | `ahe-workflow-starter` |
| `/ahe-closeout` | 完成判断与收尾 | `ahe-completion-gate` 或 `ahe-finalize` | `ahe-workflow-starter` |
...
- 若当前请求明确是规格正文产出 / 修订，且不存在 route / stage 冲突，可 direct invoke `ahe-specify`
- 若用户其实要 review spec，而不是继续写 spec，应改走 `/ahe-review` 或直接进入 `ahe-spec-review`
- 若当前已存在批准规格，且问题已经进入设计 / 任务 / 实现层，不强行停在 `/ahe-spec`
- 若用户说“先做 spec”，但当前其实混入 hotfix / increment / review-only / 阶段不清信号，回到 `ahe-workflow-starter`
...
- 若当前存在唯一活跃任务，且任务计划已批准，或已有 hotfix / 回流 handoff，可 direct invoke `ahe-test-driven-dev`
...
- 若当前请求是“继续做实现”，但 stage / profile / 工件状态冲突，回到 `ahe-workflow-starter`
...
- 若用户已经明确 review 对象，且工件存在，可直接进入对应 review / gate 节点
- 若参数使用 `trace`，默认映射到 `ahe-traceability-review`
- 若 review 对象仍模糊，例如只说“帮我 review 一下”，回到 `ahe-workflow-starter`
...
- 若 review / gate 请求暴露出 route / stage / profile 冲突，立即回 starter，不在命令层硬判
...
- 若已有 `ahe-regression-gate` 记录、实现交接块与当前完成声明，但尚未正式判断“能否算完成”，进入 `ahe-completion-gate`
- 若 `ahe-completion-gate` 已给出允许 finalize 的结论，进入 `ahe-finalize`
- 若 closeout 过程中发现仍需补实现、补验证或 gate 记录不支持 finalize，回到 `ahe-workflow-starter`
...
- `/ahe-spec add-rate-limit-rules`
  - 偏向进入 `ahe-specify`
- `/ahe-build TASK-003`
  - 若 `TASK-003` 已是唯一活跃任务且前置工件齐全，偏向进入 `ahe-test-driven-dev`
- `/ahe-review code TASK-003`
  - 偏向进入 `ahe-code-review`
- `/ahe-closeout TASK-003`
  - 若 completion gate 尚未执行，先偏向 `ahe-completion-gate`；若已通过，则偏向 `ahe-finalize`
