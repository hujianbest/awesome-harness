...
- `ahe-*` 当前已经不再只是“只能串联调用的 workflow 节点集合”。本轮修改之后，它更准确的定位是“以 `ahe-workflow-starter` 为 kernel 的软件交付 workflow family”，并开始具备“条件式独立调用 + 强约束串联调用”的双模式能力。关键依据在 `skills/ahe-workflow-starter/SKILL.md`、`skills/ahe-specify/SKILL.md`、`skills/ahe-design/SKILL.md`、`skills/ahe-tasks/SKILL.md`、`skills/ahe-test-review/SKILL.md`、`skills/ahe-bug-patterns/SKILL.md`。
- AHE 当前最突出的新变化有三点：一是 `ahe-workflow-starter` 的连续执行、迁移表和恢复编排协议更完整；二是 review 节点已经明确走 reviewer subagent 协议；三是家族开始朝“独立调用 + 串联调用”双模式收敛，但 live skills 尚未完全统一成同一套 section-level contract。
...
- `ahe-workflow-starter` 现在不仅负责起始路由，还明确承担连续执行、review/gate 后恢复编排、显式 handoff 解释和结果驱动迁移。参见 `skills/ahe-workflow-starter/SKILL.md`。
- review 节点现在有更清晰的 subagent 执行协议，尤其是 `skills/ahe-workflow-starter/references/review-dispatch-protocol.md` 和 `skills/ahe-workflow-starter/references/reviewer-return-contract.md`，说明 AHE 已经把 reviewer 从“当前会话直接继续评审”推进到“受控 reviewer return contract”。
- 多个上游节点已经统一加入“如果当前请求尚未经过 `ahe-workflow-starter` 的阶段判断，应先回到 starter 完成路由”的硬性门禁，例如 `skills/ahe-specify/SKILL.md`、`skills/ahe-design/SKILL.md`、`skills/ahe-tasks/SKILL.md`、`skills/ahe-spec-review/SKILL.md`、`skills/ahe-design-review/SKILL.md`、`skills/ahe-tasks-review/SKILL.md`。
...
- starter collateral 的最高优先级冲突已经开始被关闭，但 `ahe-workflow-starter/SKILL.md` 主文件仍然偏重，离理想的 kernel 形态还有距离。
...
| 复用单元 | 单 skill 可独立触发，也可被命令串联 | 单节点 skill 可被独立点名，但常需满足前置条件；阶段不清时由 `ahe-workflow-starter` 验证并恢复编排 | `agent-skills` 仍更像工具箱，AHE 更像流程机 |
| 路由方式 | `using-agent-skills` 元 skill + `.claude/commands/*.md` + SessionStart hook | `ahe-workflow-starter` + legal node set + migration tables + recovery protocol | AHE 路由更强约束、更状态机化 |
...
| 辅助资产 | `docs/skill-anatomy.md`、`agents/`、`.claude/commands/`、`hooks/`、`references/` | `templates/` + skill `references/` + `AGENTS.md` + starter collateral docs | `agent-skills` 配套层更产品化，AHE 配套层更 workflow-specific |
| 可移植性 | 高，默认就是可分发 skill pack | 中等；比上一版更可拆用，但仍依赖目录、记录路径与状态工件 | AHE 仍需一层 repo-agnostic 抽象 |
| 维护成本 | 中等，靠统一 anatomy 降低漂移 | 仍偏高；starter 过重，且家族规则尚未完全从 live skills 中抽离 | AHE 仍更容易出现一致性漂移 |
...
- `ahe-workflow-starter` 仍是非常强的 workflow kernel。它已经明确 profile、合法节点集合、恢复编排、连续执行、pause point 和迁移来源。参见 `skills/ahe-workflow-starter/SKILL.md`。
- reviewer subagent 协议已经开始成体系落地。`skills/ahe-workflow-starter/references/review-dispatch-protocol.md` 和 `skills/ahe-workflow-starter/references/reviewer-return-contract.md` 是本轮最重要的结构增强之一。
- AHE 不再只是“只能串联调用”。上游节点通过“先回 starter 路由”的硬性门禁明确了 direct invoke 的 fallback；部分质量节点又允许手动补充性调用。这说明 AHE 已开始形成双模式能力。
...
- starter 仍然偏重。虽然 collateral docs 增加了，但 `skills/ahe-workflow-starter/SKILL.md` 本体仍然承载了大量规则与解释。
- starter collateral 的关键冲突已经开始修补，但 family 级解释层与 live skills 之间仍有进一步对齐空间。
...
- 最高优先级结构问题从“是否支持 direct invoke”转为：
  ...
  - 修正 starter collateral 与 starter 主文件之间的冲突
  - 继续把 starter 中的解释层下沉到 `references/`
...
- 不建议为了更像 `agent-skills-main` 而弱化 `ahe-workflow-starter` 的 authority。
- 不建议在当前 starter collateral 还未对齐之前，过早增加更多 wrapper、hook 或 plugin 层。
