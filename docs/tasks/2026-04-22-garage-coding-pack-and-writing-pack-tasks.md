...
- **Files / 触碰工件**: `packs/coding/skills/<id>/` × 22（新增）
- **测试设计种子**:
...
- **Verify**:
  - `ls packs/coding/skills/ | wc -l` 应包含 22 项 + （可能含 docs/ templates/ 由 T1b 加，但 T1a 时不应有）
...
- **测试设计种子**（这是本 task 的核心交付）:
  ...
  - 关键边界：测试在 fixture 不依赖 .garage/ 或临时目录，直接读仓库内固定路径（这是 sentinel 测试的特性）
  - fail-first 适用点：先写测试 → 在 drift 同步前跑应 RED（two files differ）→ 同步后跑应 GREEN
- **Verify**:
...
- **预期证据**: PR commit `f008(coding/drift-sync): 反向同步根级 skill-anatomy.md + sentinel test` 的 git diff 含 docs/principles/skill-anatomy.md 修改 + 1 个新测试文件
- **完成条件**: drift 收敛 + sentinel 测试 GREEN + commit 落地

### T2. packs/writing/ 4 skill + LICENSE + pack.json + README

- **目标**: 把 `.agents/skills/write-blog/{blog-writing,humanizer-zh,hv-analysis,khazix-writer}/` 共 4 个 skill cp -r 到 `packs/writing/skills/`；保留上游 LICENSE；写 pack.json + README
- **Acceptance**:
  - `ls packs/writing/skills/ | wc -l` == 4
  - 任一 write-blog skill 的 SKILL.md 在源端与目的端 SHA-256 字节级相等（CON-803 / INV-4）
  ...
  - INV-9: `grep -rE '\.claude/|\.cursor/|\.opencode/|claude-code' packs/writing/` 命中 = 0
- **依赖**: 无（与 T1 系列并行）
- **Ready When**: 当前分支已就位
- **初始队列状态**: ready
- **Selection Priority**: 4（与 T1c 并行可，但建议串行以保 git diff 可审计）
- **Files / 触碰工件**:
  - 新增 `packs/writing/skills/<id>/` × 4（含各自 prompts/ examples/ 子目录）
...
- **测试设计种子**:
  ...
  - 关键边界：khazix-writer 的 prompts/ 子目录被完整搬迁
  - fail-first 适用点：T4c 集成测试 assert writing pack 存在
- **Verify**:
  - `cat packs/writing/pack.json | jq '.skills | length'` == 4
  - `test -f packs/writing/LICENSE && echo "LICENSE present"`
  - `find packs/writing/skills -name 'SKILL.md' | wc -l` == 4
  - INV-9 grep
- **预期证据**: PR commit `f008(writing): packs/writing/ 4 skill + LICENSE + pack.json + README`
- **完成条件**: 4 skill 物理存在 + LICENSE 在 + pack.json 通过 discover_packs 校验 + INV-9 通过 + commit 落地

### T3. packs/garage/ 扩容（+find-skills +writing-skills, 0.1.0→0.2.0）

- **目标**: 把 `.agents/skills/find-skills/` 与 `.agents/skills/writing-skills/`（含 examples/ render-graphs.js 等子文件）搬到 `packs/garage/skills/`；改写 `packs/garage/pack.json.skills[]` 从 `["garage-hello"]` 扩到字典序 `["find-skills", "garage-hello", "writing-skills"]`；version `0.1.0` → `0.2.0`；同步刷新 README
- **Acceptance**:
  ...
  - find-skills/SKILL.md 与 .agents/skills/find-skills/SKILL.md 字节相等
  - writing-skills/SKILL.md + examples/ + render-graphs.js + 3 reference .md 全部 1:1 搬迁（INV-4 / CON-803）
  - discover_packs 不抛错
...
- **测试设计种子**:
  ...
  - 关键边界：writing-skills 的 render-graphs.js 也被搬迁（不可执行性是 deferred 不影响 cycle）
  - fail-first 适用点：T4c `test_packs_garage_extended.py` assert
- **Verify**:
...
- **预期证据**: PR commit `f008(garage): +find-skills +writing-skills, 0.1.0→0.2.0`
- **完成条件**: 3 skill 物理存在 + pack.json 字段更新 + commit 落地

### T4a. .agents/skills/ 删除 + .gitignore 排除 dogfood 产物

- **目标**: `rm -rf .agents/skills/`；`.gitignore` 新增 `.cursor/skills/` + `.claude/skills/` 两行
- **Acceptance**:
  - `ls .agents/skills/` 报错 "No such file or directory"
  - `git status --porcelain` 输出空（INV-6 / 红线 2）— 注意：此时 packs/{coding,writing,garage}/ 已落地（T1-T3），如果还没装 dogfood，.cursor/skills 与 .claude/skills 不存在；如果装了但未提交，需要在 .gitignore 排除它们
...
- **目标**: (a) `## Packs & Host Installer` 段 5 分钟冷读指针表中 "F008 候选" → "已落地（F008）"；(b) 新增 "本仓库自身 IDE 加载入口" 段，说明首次 clone 贡献者必须先跑 `garage init --hosts cursor,claude` 激活 dogfood 产物（ADR-D8-2 候选 C 配套）
- **Acceptance**:
  - `grep -A2 'Packs & Host Installer' AGENTS.md` 含 "已落地" 而不是 "候选"
  - AGENTS.md 含至少一段 `garage init --hosts cursor,claude` 形式的命令样板
  - `grep 'docs/principles/skill-anatomy.md' AGENTS.md` 仍命中（红线 4：冷读链不破）
  - 现有 AGENTS.md § "Skill 写作原则" 段未被破坏
...
- **Verify**:
  - `grep -E 'Packs & Host Installer' AGENTS.md`
  - `grep -E 'garage init --hosts' AGENTS.md`
  - `grep -E 'docs/principles/skill-anatomy.md' AGENTS.md`（红线 4 守门）
- **预期证据**: PR commit `f008(layout/agents-md): AGENTS.md Packs 段刷新 + 首次 clone 贡献者 onboarding`，git diff 显示 AGENTS.md 局部修改
- **完成条件**: AGENTS.md 三个 invariant 通过 + commit 落地
...
- **Files / 触碰工件**:
  ...
  - 修改 `RELEASE_NOTES.md`（新增 F008 段，占位 — 实测数据由 finalize 填）
- **测试设计种子**:
  - 主行为：3 处文档 grep 检查
...
**关键路径**: T1a → T1b → T1c → T4a → T4b → T4c → T5（7 跳）；T2 + T3 与 T1c 并行但建议串行落 commit（NFR-804 git diff 可审计）

**关键约束**:
- T4a 必须在 T1+T2+T3 全部完成之后（packs/ 已就位才能删 .agents/skills/）
...
6. End-to-end smoke 在 PR walkthrough 提供证据（dogfood `garage init --hosts cursor,claude` + `find .cursor/skills | head` + 截图）
