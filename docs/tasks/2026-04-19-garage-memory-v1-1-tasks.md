...
- **测试设计种子（hf-test-driven-dev；fail-first）**:
  ...
  7. **NFR-402 性能**: 在 `tests/memory/` baseline 时长基础上跑 N=3 次取均值，T1 ≤ 1.1 * T0
- **Verify**:
...
- **依赖**: -（与 T1/T2/T3 解耦，可与 T1 并行；但实施顺序优先 T1 → T2 → T3 → T4）
- **Ready When**: F004 spec/design approval 已完成
- **初始队列状态**: pending（按实施顺序排在 T3 后）
...
启动时：T1 是唯一 P1 ready，选 T1。

T1 完成后（router 重选）：T2 因 `Depends On=T1` 与 `Ready When=T1=done` 满足而 ready；T4 此时也 ready（无依赖、Priority P2）。两者都 ready 但 T2 Priority 更高（P1 > P2）→ 选 T2。
...
**文档状态**: 草稿，待 `hf-tasks-review`。

**下一步**: 派发 reviewer 执行 `hf-tasks-review`。
